        ...
        if (check_sprdimgheader((uint8_t *)&img_header, ram_addr) != 0) {
            ...
            if (hdr->flags != 0) {
                ...
                if (check_sprdimgheader((uint8_t *)&img_header, ram_addr) != 0) {
                    ...
                    printf("check vbmeta sprd sign image header magic number error!\n");
                    goto error;
                }
            }
        }
...
#endif
#endif
    //FIXME skip AntiRollback check for socko because overflow
#ifndef CONFIG_K54_FLAG
...
fail:
	errorf("verify the vbmeta error\n");
	while (1);
}
...
void vboot_secure_process_flow(char *partition_name)
{
	...
	if((strcmp("vbmeta_system",partition_name) == 0) || (strcmp("vbmeta_vendor",partition_name) == 0) ||
           (strcmp("vbmeta_system_ext",partition_name) == 0) || (strcmp("vbmeta_product",partition_name) == 0)) {
	...
	if ((strcmp("boot",partition_name) == 0) || (strcmp("recovery",partition_name) == 0))
	{
		...
		debugf("bootimage and recovery addr is 0x%x, buf_size is %d\n",buf_base,buf_size);
	...
	else if (strcmp("vendor_boot",partition_name) == 0)
	{
		...
		debugf("vendor_boot addr is 0x%x, buf_size is %d\n",buf_base,buf_size);
		temp_base = vboot_verify_info->img_addr;
