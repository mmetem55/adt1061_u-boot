...
uint64_t ddr_img_addr = 0;
uint64_t vendorboot_ddr_img_addr = 0;
...
int block_dev_read(block_dev_desc_t *dev_desc, uint64_t size, uint64_t offset, char *buf)
{
	...
	if (0 != offb) {
		...
		if (1 != dev_desc->block_read(dev_desc->dev, start, 1, temp)) {
			errorf("read start block 0x"LBAF" fail\n", start);
			goto fail;
		}
		if (size <= header_sz) {
	...
	if (blkcnt > 0) {
		if (blkcnt != dev_desc->block_read(dev_desc->dev, start, blkcnt, buf)) {
			errorf("read block 0x%llx size 0x"LBAF" fail\n", start, blkcnt);
			goto fail;
		}
		start += blkcnt;
	...
	if (size > 0) {
		if (1 != dev_desc->block_read(dev_desc->dev, start, 1, temp)) {
			errorf("read end block 0x"LBAF" fail\n", start);
			goto fail;
		}
...
int block_dev_write(block_dev_desc_t *dev_desc, uint64_t size, uint64_t offset, char *buf)
{
	...
	if (0 != offb) {
		header_sz = dev_desc->blksz - offb;
		if (1 != dev_desc->block_read(dev_desc->dev, start, 1, temp)) {
			errorf("read start block 0x"LBAF" for write fail\n", start);
			goto fail;
		}
		if (size <= header_sz) {
		...
		if (1 != dev_desc->block_write(dev_desc->dev, start, 1, temp)) {
			errorf("write start block 0x"LBAF" fail\n", start);
			goto fail;
		}
		start++;
	}
	...
	if (blkcnt > 0) {
		if (blkcnt != dev_desc->block_write(dev_desc->dev, start, blkcnt, buf)) {
			errorf("write block 0x"LBAF" size 0x"LBAF" fail\n", start, blkcnt);
			goto fail;
		}
		start += blkcnt;
		buf += (blkcnt * dev_desc->blksz);
	...
	if (size > 0) {
		if (1 != dev_desc->block_read(dev_desc->dev, start, 1, temp)) {
			errorf("read end block 0x"LBAF" for write fail\n", start);
			goto fail;
		}
		memcpy(temp, buf, size);
		if (1 != dev_desc->block_write(dev_desc->dev, start, 1, temp)) {
			errorf("write end block 0x"LBAF" fail\n", start);
			goto fail;
		}
	}
...
int block_dev_erase(block_dev_desc_t *dev_desc, uint64_t size, uint64_t offset)
{
	...
	if (0 != offb) {
		header_sz = dev_desc->blksz - offb;
		if (1 != dev_desc->block_read(dev_desc->dev, start, 1, temp)) {
			errorf("read start block 0x"LBAF" for erase fail\n", start);
			goto fail;
		}
		if (size <= header_sz) {
		...
		if (1 != dev_desc->block_write(dev_desc->dev, start, 1, temp)) {
			errorf("erase start block 0x"LBAF" fail\n", start);
			goto fail;
		}
		start++;
	}
	...
	if (blkcnt > 0) {
		if (blkcnt != dev_desc->block_erase(dev_desc->dev, start, blkcnt)) {
			errorf("erase block 0x"LBAF" size 0x"LBAF" fail\n", start, blkcnt);
			goto fail;
		}
		start += blkcnt;
		size = size % dev_desc->blksz;
	...
	if (size > 0) {
		if (1 != dev_desc->block_read(dev_desc->dev, start, 1, temp)) {
			errorf("read end block 0x"LBAF" for erase fail\n", start);
			goto fail;
		}
		memset(temp, 0, size);
		if (1 != dev_desc->block_write(dev_desc->dev, start, 1, temp)) {
			errorf("erase end block 0x"LBAF" fail\n", start);
			goto fail;
		}
	}
...
int common_raw_read(char *part_name, uint64_t size, uint64_t offset, char *buf)
{
	...
	int ret = 0;
	uchar part_name_temp[MAX_PARTITION_NAME_SIZE];

#ifdef CONFIG_ANDROID_AB
	if (ddr_img_addr && !strncmp(part_name, "boot_", 5))
#else
	if (ddr_img_addr && (!strcmp(part_name, "boot")))
#endif
	{
		debugf("the second boot.img and recovery.img load is in DDR during secboot mode\n");
		offset += ddr_img_addr;
...
#ifdef CONFIG_ANDROID_AB
        if (vendorboot_ddr_img_addr && !strncmp(part_name, "vendor_boot_", 12))
#else
        if (vendorboot_ddr_img_addr && (!strcmp(part_name, "vendor_boot")))
#endif
        {
                debugf("the second vendor_boot.img load is in DDR during secboot mode\n");
                offset += vendorboot_ddr_img_addr;
                memcpy(buf, offset, size);
                return 0;
        }
	...
	}else if (0 != strstr(part_name, "fixnv")){
		dev_id = get_devnum_hwpart(ifname, USER_PART);
		dev_desc = get_dev_hwpart(ifname, dev_id, USER_PART);
		ret = get_partition_info_by_name(dev_desc, part_name, &part_info);
		if (-1 == ret) {
			ret = 0;
			memset(part_name_temp, 0, MAX_PARTITION_NAME_SIZE);
			if (0 != strstr(part_name, "_a")) {
				memcpy(part_name_temp, part_name, strlen(part_name)-2);

			}else {
				sprintf(part_name_temp, "%s%s", part_name, "_a");
			}
			debugf("try to adapt ab and not ab fix partition name: %s\n", part_name_temp);
			ret = get_partition_info_by_name(dev_desc, part_name_temp, &part_info);
			part_start = (uint64_t)part_info.start * (uint64_t)part_info.blksz;
			total_part_size = (uint64_t)part_info.blksz * (uint64_t)part_info.size;
			if (size + offset > total_part_size) {
...
int common_write_backstage(char *part_name, uint32_t size, uint64_t offset, char *buf)
{
	...
	if (0 != size % local_part_info.blksz || 0 != offset % local_part_info.blksz) {
		errorf("input size or offset not aligned, size(0x%x), offset(0x%llx)\n", size, offset);
		ret = -1;
	}
...
static int get_block_dev_desc(block_dev_desc_t **dev_desc,
			const char *raw_partition, u64 offset, u64 size)
{
	int dev_id = 0;
	char *ifname;
	block_dev_desc_t *desc;
	u64 total_part_size = 0;

	ifname = block_dev_get_name();

	if (!strcmp(raw_partition, "user_part")) {
		dev_id = get_devnum_hwpart(ifname, USER_PART);
		if (dev_id < 0) {
			errorf("get user part dev num fail!\n");
			return dev_id;
		...
		desc = get_dev_hwpart(ifname, dev_id, USER_PART);
		if (!desc) {
			errorf("get user part dev fail!\n");
			return -ENODEV;
		}
		...
		total_part_size = get_devsize_hwpart(ifname, USER_PART);
		if (size + offset > total_part_size) {
			errorf("user part size(0x%llx + 0x%llx)\
				overflow the total partition size(0x%llx)\n",
				size, offset, total_part_size);
			return -EINVAL;
		}
	} else if (!strcmp(raw_partition, "boot_part1")) {
		dev_id = get_devnum_hwpart(ifname, BOOT_PART1);
		if (dev_id < 0) {
			errorf("get boot part1 dev num fail!\n");
			return dev_id;
		}

		desc = get_dev_hwpart(ifname, dev_id, BOOT_PART1);
		if (!desc) {
			errorf("get boot part1 dev fail!\n");
			return -ENODEV;
		}

		total_part_size = get_devsize_hwpart(ifname, BOOT_PART1);
		if (size + offset > total_part_size) {
			errorf("boot part1 size(0x%llx + 0x%llx)\
				overflow the total partition size(0x%llx)\n",
				size, offset, total_part_size);
			return -EINVAL;
		}
	} else if (!strcmp(raw_partition, "boot_part2")) {
		dev_id = get_devnum_hwpart(ifname, BOOT_PART2);
		if (dev_id < 0) {
			errorf("get boot part2 dev num fail!\n");
			return dev_id;
		}

		desc = get_dev_hwpart(ifname, dev_id, BOOT_PART2);
		if (!desc) {
			errorf("get boot part2 dev fail!\n");
			return -ENODEV;
		}

		total_part_size = get_devsize_hwpart(ifname, BOOT_PART2);
		if (size + offset > total_part_size) {
			errorf("boot part2 size(0x%llx + 0x%llx)\
				overflow the total partition size(0x%llx)\n",
				size, offset, total_part_size);
			return -EINVAL;
		}
	} else {
		errorf("no support partition: %s\n", raw_partition);
		return -ENOSPC;
	}
...
int raw_read(char *raw_partition, u64 offset, u64 size, char *buf)
{
	int ret = 0;
	block_dev_desc_t *dev_desc = NULL;

	if (size == 0) {
		errorf("size is equal to zero!\n");
		return -EINVAL;
	}

	ret = get_block_dev_desc(&dev_desc, raw_partition, offset, size);
	if (ret)
		return ret;
...
int raw_write(char *raw_partition, u64 offset, u64 size, char *buf)
{
	int ret = 0;
	block_dev_desc_t *dev_desc = NULL;
