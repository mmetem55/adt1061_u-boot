...
int merge_bootargs(u8 *fdt_blob)
{
	...
	sprintf(buf, "androidboot.efused=%d", s_efused);
	ret = fdt_chosen_bootargs_append(fdt_blob, buf, 1);
	if (ret) {
...
*/
		...
		case v_state_green:
			...
			debugf("WARNNING: s_is6000F=%d\n", s_is6000F);

			if ( s_boot_mode == CMD_NORMAL_MODE && s_is6000F != 1 ) {
				lcd_printf("\nUserdebug version is not allow to bootup on the locked tablet. Tablet will be powered off !!!\n\n");
