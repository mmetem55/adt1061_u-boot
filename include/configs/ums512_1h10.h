...
/* ADT1061 (aidata) is a NON-A/B device: single "boot"/"recovery"/"vbmeta"
 * partitions (see ums512_1h10.xml). Defining CONFIG_ANDROID_AB here forced U-Boot
 * to append "_a" slot suffix (boot->boot_a, l_agdsp->l_agdsp_a, ...) which do not
 * exist on this device's GPT, so normal boot failed with
 *   _get_kernel_ramdisk_dt_offset(): read boot_a header error!
 * and fell through to "cboot fastboot". Disable A/B so partitions are read by
 * their real names (boot/l_agdsp/...). */
