...
static int lcd_id[3]={16,15,115};
static get_id[3]={0};
static int lcd_version;
char *lcd_id_version;
static void request_gpios(void)
{
	int i;
	for(i = 0;i < 3;i++)
	sprd_gpio_request(NULL, lcd_id[i]);
	return;
}
static void gpio_getid(void)
{
	int i;
	for(i = 0;i < 3;i++)
	{
...
static void dump_panel_info(struct panel_info*  info_data)
{
	...
	pr_info("lcd: gpio_tp3v3en: %d \n", info->power_pin.gpio_tp3v3en);
...
// add end
	...
	if (0xFFFF == info->lcd_id && lcd_version ==0) {
		pr_info("Dummy LCD lcd_version =%d\n",lcd_version);
	
		if(read_lcd_buf[0] ==0x84)
			lcd_id_version ="lc0";
		else if(read_lcd_buf[0] ==0x85)
			lcd_id_version ="lc1";
		else
			lcd_id_version ="lc0";
		return 0;
	...
	if(lcd_version ==1){
		lcd_id_version ="djn";
		if(read_lcd_buf[0] != 0x01 && read_lcd_buf[0] != 0x00 && 0xFFFC == info->lcd_id)
		return 0;
		if(0xFFFD == info->lcd_id )
		return 0;	
		
	}
	
	if(lcd_version ==3){
		lcd_id_version ="XBdjn";
		if(0xFFFE == info->lcd_id )
		return 0;	
	...
	if (!id->reg_items) {
		...
		/* Bringup force-attach: a real panel on this board has an
		   explicit lcd-id-tokernel but no MIPI-readable ID register,
		   so it can't be matched the normal way. Accept it here as long
		   as the tokernel value is NOT one of the reserved GPIO-version
		   shortcuts (0xfffc/0xfffd/0xfffe/0xffff) handled above — most
		   importantly 0xffff, which is what the lcd_dummy_mipi_hd panel
		   uses; the dummy is probed first and must NOT be accepted on
		   a real-panel board, otherwise probe stops before the real
		   panel and the kernel gets lcd_id=IDffff. */
		if (info->lcd_id != 0 &&
		    info->lcd_id != 0xffff && info->lcd_id != 0xfffe &&
		    info->lcd_id != 0xfffd && info->lcd_id != 0xfffc) {
			pr_err("force-attach panel '%s' by tokernel=0x%x\n",
				info->lcd_name, info->lcd_id);
			return 0;
		}
		return -1;
