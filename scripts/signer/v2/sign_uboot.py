...
def load_config(db_path: Path) -> dict:
    ...
    for key, val_int, val_blob in cursor.execute("SELECT key, val_int, val_blob FROM config"):
        cfg[key] = val_int if val_int is not None else val_blob
    conn.close()
    return cfg
...
TEE_BLOCK = CONFIG["TEE_BLOCK"]
SIGNATURE = CONFIG["SIGNATURE"]
HEADER_SIZE = CONFIG["HEADER_SIZE"]
TOTAL_SIZE_FIELD_OFFSET = CONFIG["TOTAL_SIZE_FIELD_OFFSET"]
UBOOT_SIZE_FIELD_OFFSET = CONFIG["UBOOT_SIZE_FIELD_OFFSET"]
TABLE_SIZE = CONFIG["TABLE_SIZE"]
TEE_PAD = CONFIG["TEE_PAD"]
MAGIC = CONFIG["MAGIC"]
PAD_ALIGN = CONFIG["PAD_ALIGN"]
PAD_BYTE = CONFIG["PAD_BYTE"]
